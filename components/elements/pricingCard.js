...
import { stripeCheckout } from "@/service/stripe/stripe-checkout";
import { redirectToCheckout } from "@/service/stripe/stripe-redirect";

export default function PricingCard({ tier, userEmail }) {
  useEffect(() => {
    ...
    <>
      <div className="flex flex-col items-center aspect-auto p-4 sm:p-8 border rounded-3xl bg-white dark:bg-gray-800 border-gray-700 shadow-gray-600/10 shadow-none m-2 flex-1 max-w-md">
        ...
        {tier.paymentProvider == "LemonSqueezy" ? (
          ...
          <Button
            onClickEvent={async () => {
              const lineItems = tier.stripe.lineItems;
              const checkoutType = tier.stripe.checkoutType;
              const sessionId = await stripeCheckout({ lineItems, userEmail, checkoutType });
